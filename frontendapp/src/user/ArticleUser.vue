<template>
    <div class="article-page-container">
        ...
        <h4>Activities</h4>
        <ul>
            <li v-for="(activity, index) in activities" :key="index" @click="goAt(activity)">{{ activity }}</li>
        </ul>
...
export default {
    ...
    methods: {
        ...
        async fetchAll() {
            ...
            this.activities = await (await fetch('http://localhost:8080/api/articles/activities/' + this.article.articleId)).json();
            this.comments = await (await fetch('http://localhost:8080/api/articles/comments/' + this.article.articleId)).json();
            console.log(this.article);
        },
        goAt(activity){
            this.$router.push( { name: 'activityUser', params: {activity : activity } });
        },
        addComment() {
